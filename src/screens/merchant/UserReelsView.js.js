...
const UserReelsView = () => {
     ...
     const fetchMerchantReelsDirect = async (pageNum = 1, perPage = 20) => {
          try {
               const response = await getData(`${ROUTES.MERCHANT_MY_REELS}?page=${pageNum}&per_page=${perPage}`);
               return response;
          } catch (error) {
     ...
     return (
          <View style={[styles.container, { backgroundColor: '#000' }]}>
               ...
               {showLoading ? (
                    ...
                    <View style={styles.loadingContainer}>
                         <Text style={styles.loadingText}>No reels found. Start creating your first reel!</Text>
                    </View>
               )}
          </View>
     );
