...
function* fetchMerchantReelsWorker(action) {
  try {
    ...
    if (response && response.status === 'success' && response.data) {
      ...
      const mappedReels = response.data.map(apiReel =>
        mapReelData(apiReel, currentUser),
      );
...
// Map API response to component format for public reels
const mapPublicReelData = (apiReel, currentUser = null) => {
  // For public reels, show the creator's info (merchant who created the reel)
  // If API provides merchant info, use it; otherwise use product name
  // If logged-in user is viewing their own reel, show their username
  const userInfo = currentUser?.data || currentUser || {};
...
function* fetchPublicReelsWorker(action) {
  try {
    ...
    console.log('Fetching public reels:', {page, perPage});

    // Call API with pagination params
    const response = yield call(getData, ROUTES.PUBLIC_REELS, {
      page,
      per_page: perPage,
    });
    ...
    if (response && response.status === 'success' && response.data) {
      // Map API response to component format with current user data
      const mappedReels = response.data.map(apiReel =>
        mapPublicReelData(apiReel, currentUser),
      );
